...
(function () {
  ...
  // дошло ли оно. Опрашиваем статус, пока не увидим 'sent'/'failed'. 'failed' — это
  // настоящий, окончательный провал (воркер исчерпал все попытки, см.
  // VerifyEmailWithCode::failed()) — тут предупреждаем. А вот истечение времени
  // ожидания БЕЗ 'failed' — это просто "ещё не знаем" (полный путь джоба → воркер
  // её забрал → SMTP-рукопожатие → событие может занять больше ~20 сек и без какой-
  // либо проблемы), а не "точно не отправлено" — молча прекращаем опрос без
  // тревожного сообщения, чтобы не провоцировать пользователя нажимать "отправить
  // снова" и тем самым инвалидировать уже верный, просто ещё не подтверждённый код.
  const warning = document.getElementById('email-delivery-warning');
  const POLL_MS = 2000;
  const MAX_POLLS = 25; // ~50 сек — с запасом на sleep воркера + SMTP-рукопожатие
  let polls = 0;
  ...
  function poll() {
    fetch('{{ route('auth.email.status') }}', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(data => {
        ...
        if (++polls >= MAX_POLLS) return; // тихо прекращаем — не значит, что не отправлено
