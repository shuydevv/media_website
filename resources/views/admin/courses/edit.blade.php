...
<div class="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-xl">
    <h1 class="text-2xl font-bold mb-6">Редактировать курс</h1>
    ...
    <form method="POST" action="{{ route('admin.courses.update', $course->id) }}" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="w-full border rounded px-3 py-2" value="{{ old('title', $course->title) }}" required>
        </div>
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="start_date" id="start_date" class="w-full border rounded px-3 py-2" value="{{ old('start_date', $course->start_date) }}" required>
            </div>
            <div>
                <label for="end_date" class="block font-medium text-sm">Дата окончания</label>
                <input type="date" name="end_date" id="end_date" class="w-full border rounded px-3 py-2" value="{{ old('end_date', $course->end_date) }}" required>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <div id="schedule-wrapper">
                @foreach ($course->scheduleTemplates as $i => $item)
        ...
        <div class="mb-4">
            <label for="content" class="block text-sm font-medium">Контент</label>
            <textarea name="content" id="content" class="w-full border rounded px-3 py-2">{{ old('content', $course->content) }}</textarea>
        </div>

        <div class="mb-4">
            <label for="path" class="block text-sm font-medium">URL (Путь)</label>
            <input type="text" name="path" id="path" value="{{ old('path', $course->path) }}" class="w-full border rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label for="html_title" class="block text-sm font-medium">HTML Title</label>
            <input type="text" name="html_title" id="html_title" value="{{ old('html_title', $course->html_title) }}" class="w-full border rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label for="html_description" class="block text-sm font-medium">HTML Description</label>
            <input type="text" name="html_description" id="html_description" value="{{ old('html_description', $course->html_description) }}" class="w-full border rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label for="main_image" class="block text-sm font-medium">Обложка курса (оставьте пустым, чтобы не менять)</label>
